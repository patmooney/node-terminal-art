 ...
 * @param {number} [options.frameIdx=0] - Only really applicable to gifs
 * @returns {Promise<[][]>} ansii 2d arr
 */
async function processImage(img, { maxCharWidth = null, minDist = 5, frameIdx = 0, previousImg = null, storeAllPixels = false } = {}) {
    const { pixels, frames, width, height, channels, frameSize } = img;

    maxCharWidth = maxCharWidth || process.stdout.columns * 0.75;
    frameIdx = Math.min(frameIdx, frames);
    ...
    function getAnsiiCode(colours, colourStr) {
        ...
        return [
            ansii,
            ansii === previousAnsii && !storeAllPixels
                ? ' '
                : colourOctal(ansii) + ' '
        ];
    }
    ...
    const output = [];
    const maxHeight = Math.ceil(height / yOffset);
    const maxWidth = Math.ceil(width / xOffset);
    for (let y = 0; y < maxHeight; y++) {
        const row = [];
        for (let x = 0; x < maxWidth; x++) {
            const offset = (y * width * channels * yOffset) + (x * channels * xOffset) + (frameIdx * frameSize);
            const colours = pixels.data.slice(offset, offset + channels);
            const colourStr = colours.join('.');
            const [ansii, ansiiCode] = getAnsiiCode(colours, colourStr);
            if (previousImg && ansii === null) {
                row.push(previousImg[y][x]);
            } else {
                row.push(ansiiCode);
            }
...
async function getImageFrames(path, options) {
    if (path instanceof Buffer && !options.mimeType) {
        throw new Error('If a buffer is provided, so should be the mimeType as part of the options');
    }
    const img = await loadImage(path, options.mimeType);
    const frames = [];
    for (let frameIdx = 0; frameIdx < img.frames; frameIdx++) {
        frames.push(
            await processImage(
                img,
                {
                    ...options,
                    previousImg: frameIdx > 0 ? frames[frameIdx - 1] : null,
                    frameIdx
                }
            )
        );
...
async function gifToAnsii(path, options) {
    const frames = await getImageFrames(path, { ...options, storeAllPixels: true });
    return frames.map(
        frame => frame.map(
            row => row.join('')
        ).join(cS(0) + nL) + cS(0)
...
async function imageToAnsii(path, options) {
    if (path instanceof Buffer && !options.mimeType) {
        throw new Error('If a buffer is provided, so should be the mimeType as part of the options');
    }
    const img = await loadImage(path, options.mimeType);
    const frame = await processImage(img, options);
    return frame.map(
        row => row.join('')
    ).join(cS(0) + nL) + cS(0);
};

async function loadImage(path, mimeType = null) {
    const pixels = await getPixels(path, mimeType);
    const [frames, width, height, channels] = pixels.shape.length === 3
        ? [1, ...pixels.shape]
        : pixels.shape;
