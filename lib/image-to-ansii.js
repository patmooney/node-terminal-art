 ...
 *
 * @param {string} path
 * @param {number} [maxCharWidth=maxPossible] - how many characters wide should
 * the image be
 * @returns {Promise.<string>} ansii
 */
module.exports = async function imageToAnsii(path, maxCharWidth = null) {
    // limit image width
    maxCharWidth = maxCharWidth || process.stdout.columns * 0.75;
    const pixels2d = await readImage(path);
