 ...
 * of the colours within. Effectively returning a console-compatible
 * low-resolution copy of the inputted image
 * @param {string} path
 * @param {number} [maxCharWidth=40] - how many characters wide should
 * the image be
 * @returns {string} ansii
 */
module.exports = async function imageToAnsii(path, maxCharWidth = 40) {
    const pixels2d = await readImage(path);

    let xOffset = 1;
    let yOffset = 1;
    if (pixels2d[0].length > maxCharWidth) {
        xOffset = parseInt(pixels2d[0].length / maxCharWidth);
