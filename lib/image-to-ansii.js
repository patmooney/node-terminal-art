...
module.exports = async function imageToAnsii(path, { maxCharWidth = null, mimeType = null } = {}) {
    ...
    yOffset = parseInt(
        (((xOffset / pixels2d[0].length) * pixels2d.length) * 2) * (pixels2d[0].length / pixels2d.length)
    );
    ...
    for (let y = 0; y < pixels2d.length; y += (yOffset || 1)) {
        for (let x = 0; x < pixels2d[y].length; x += (xOffset || 1)) {
            const colours = applyAlpha(pixels2d[y][x]);
            const ansii = pixelToAnsii(colours);
            if (ansii == previousAnsii) {
                output += " ";
            }
            else {
                output += "\033[0m\033[48;5;" + ansii + "m\033[38;5;" + ansii + "m" + " ";
                previousAnsii = ansii;
            }
        }
        output += "\033[0m\n";
...
function applyAlpha(colours) {
    ...
    return [addAlpha(colours[0]), addAlpha(colours[1]), addAlpha(colours[2]), normalisedAlpha];
}
